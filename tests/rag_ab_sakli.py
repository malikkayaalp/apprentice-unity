"""RAG A/B olcumu (SAKLI hedef): dosya adi ipucu VERMEZKEN `ara` ne kazandiriyor?

    python tests/rag_ab_sakli.py [--dosya 120]   # Ollama + bge-m3 gerekir (GPU)

rag_ab.py'den turetildi; tek fark: hedef fonksiyon ayri "fiyatlandirma.py" dosyasinda degil,
tekduze modul_XX.py dosyalarindan birinin (modul_61) ICINE gomulu. list_files isimden bulamaz;
ya korlemesine okunur ya `ara` ile bulunur. Olculdu (2026-08-23): isimli hedefte ara'nin kazanci
dur-sinyali duzeltmesinden sonra kayboldu cunku dosya adi cevabi ele veriyordu - asil RAG
senaryosu bu dosyadaki.
...
Sonuc: tests/rag_sakli.son.json  |  Iddia: B'de okunan token belirgin dusmeli. Cikmazsa RAG'in
...
KOK = os.path.join(HOME, "rag_sakli")
HEDEF_MODUL = 61          # hedef bu gurultu dosyasinin icine gomulur
...
def depo_kur(hedef_klasor: str, n_dosya: int):
    ...
    hedef_gövde = HEDEF.split("\n", 1)[1].lstrip()    # modul docstring'i (ilk satir) atilir
    for i in range(n_dosya):
        ...
        with open(os.path.join(hedef_klasor, "modul", ad + ".py"), "w",
            ...
            if i == HEDEF_MODUL:      # hedef, tekduze bir dosyanin ICINE gomulur
                f.write("\n\n" + hedef_gövde)
...
import sys
sys.path.insert(0, ".")
from modul.modul_61 import kupon_indirimi_uygula as k
out = []
def kontrol(ad, fn):
    try:
...
for p in glob.glob("modul/modul_*.py"):
    if p.replace(chr(92), "/").endswith("modul_61.py"):
        continue                       # hedefin evi; degismesi gereken dosya bu
...
def kosu(c: Client, etiket: str, klasor_adi: str, gorev: str, ara_kapali: bool) -> dict:
    args = {"gorev": gorev, "kabul_kriterleri": KRITERLER, "ortam": "code",
            "calisma_dizini": os.path.join("rag_sakli", klasor_adi), "dogrulama": "derleme"}
    if ara_kapali:
...
def main() -> int:
    ...
    ap.add_argument("--dosya", type=int, default=120, help="gurultu dosyasi sayisi")
    a = ap.parse_args()
    ...
    yol = os.path.join(ROOT, "tests", "rag_sakli.son.json")
